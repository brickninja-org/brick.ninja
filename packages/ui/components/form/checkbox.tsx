...
export const Checkbox: FC<CheckboxProps> = ({ ref, checked, defaultChecked, formValue, indeterminate = false, onChange, name, disabled, children }) => {
  ...
  return (
    <label htmlFor={id} className="inline-flex flex-row items-center gap-2.5 py-2 px-4 rounded-sm leading-normal cursor-pointer select-none hover:bg-gray-100" tabIndex={0} onKeyDown={labelOnKeyDown} ref={ref} aria-disabled={disabled}>
      ...
      <div className="peer-checked:border-blue-800 peer-indeterminate:border-blue-800 peer-checked:bg-blue-800 peer-indeterminate:bg-blue-800 peer-checked:text-white peer-checked:[stroke-dashoffset:0] peer-checked:transition-[stroke-dashoffset] peer-checked:duration-300 relative flex items-center justify-center w-5 h-5 m-0 rounded-sm border-2 bg-white text-transparent transition-colors [stroke-dasharray:20_20] [stroke-dashoffset:20] appearance-none [--icon-size:1em]"><Icon icon="checkmark"/></div>
