...
async function build() {
  ...
  const iconFiles = (await readdir('raw')).filter((file) => file.endsWith('.svg'));
  ...
  for(const file of iconFiles) {
    ...
    const input = await readFile(join('raw', file), { encoding: 'utf-8' });
  ...
  await writeFile(join('dist', 'sprite.svg'), '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><defs>\n' + Object.values(icons).join('\n') + '\n</defs></svg>');
