...
export const EntityIcon: FC<EntityIconProps> = ({ icon, size = 64, type, className }) => {
  ...
  return (
    <span className={cn('[grid-area:icon] inline-flex items-center justify-center bg-background border-(--color-border-dark) w-fit shrink-0 rounded-xs aspect-square overflow-hidden', className)} data-icon-type={type}>
      <img
        loading="lazy"
        decoding="async"
        ref={handleRef}
        ...
        style={style}
        className={cn(loading && 'loading', 'max-h-full w-fit align-middle')}
        onLoad={handleLoad}/>
    </span>
  );
};
