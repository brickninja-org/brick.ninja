...
async function MarketingPage() {
  ...
  return (
    ...
    >
      <PageView page="/"/>
      <Suspense fallback={<div className="flex justify-center gap-[32px_64px] min-h-24 flex-wrap -mt-4 mb-8 -mx-4 py-8 px-4 bg-surface-3"/>}>
        <DbStats/>
      </Suspense>
...
const Stat: FC<{ href: string, title: ReactNode, value: number }> = ({ href, title, value }) => {
  return (
    <Link href={href} className="text-lg text-accent-soft-foreground hover:text-accent-soft-hover sm:text-2xl"><span className="inline font-medium text-xl sm:text-4xl"><FormatNumber value={value}/></span> {title}</Link>
  );
};
...
async function DbStats() {
  ...
  return (
    <div className="flex flex-wrap justify-center gap-[16px_32px] min-h-[82px] -mt-4 mb-8 -mx-4 py-8 px-4 bg-accent-soft max-sm:flex-col max-sm:items-center sm:min-h-[100px] sm:gap-[32px_64px]">
