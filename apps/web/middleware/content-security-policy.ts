...
export const contentSecurityPolicyMiddleware: NextMiddleware = async (request, next, data) => {
  ...
  const cspHeader = `
    ...
    connect-src 'self' ${alternateLanguageDomains.join(' ')} brickset.com rebrickable.com bn2me.vercel.app brick-ninja-api.vercel.app iconify.design unisvg.com simplesvg.com;
    font-src 'self';
    object-src 'none';
    base-uri 'self';
    form-action 'self';
